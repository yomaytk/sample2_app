class User < ApplicationRecord
  ...
  include Gravtastic
	...
	def authenticated?(attribute, token)
		...
		if digest.nil? 				# <-- if remember_digeset.nil?   ok
			false
		else	
	...
	def send_follower_notification_email(following)
		UserMailer.follower_notification(self, following).deliver_now
	end
	...
	def message_feed(you)
		Micropost.where("(user_id = :me_id AND messages_to_id = :you_id) OR \ 
												(user_id = :you_id AND messages_to_id = :me_id)", me_id: id, you_id: you.id)
	end
	...
	def unfollow(other_user)
		...
		# following.delete(other_user) <-- NG???
	end
